...
public:
  ...
  void displayFrame(const cv::Mat &frame,
                    const std::string &windowName = "FlightPath");
   ...
   * @return true if successful, false otherwise
   */
  bool initWriter(const std::string &outputPath, double fps,
                  cv::Size frameSize);
  ...
  /**
   * @brief Crop frame to center ROI if it exceeds max dimensions
   * @param frame Input frame to crop
   * @param maxWidth Maximum width (crops if frame is wider)
   * @param maxHeight Maximum height (crops if frame is taller)
   * @return Cropped frame (or original if no cropping needed)
   */
  static cv::Mat cropToCenter(const cv::Mat &frame, int maxWidth,
                              int maxHeight);
