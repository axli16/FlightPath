...
PathPlanner::createOccupancyGrid(const std::vector<Detection> &detections,
  ...
  for (const auto &det : detections) {
    ...
    // Optimization: Replace O(N^2) pixel-by-pixel assignment with vectorized
    // OpenCV rectangle fill Note: cv::rectangle with cv::FILLED is inclusive of
...
void PathPlanner::maskOccupancyGrid(cv::Mat &grid, const cv::Rect &roi,
  ...
  // Optimization: Replace O(N^2) nested loop with natively vectorized OpenCV
  // operation
