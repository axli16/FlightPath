...
PathPlanner::createOccupancyGrid(const std::vector<Detection> &detections,
  ...
  for (const auto &det : detections) {
    ...
    // Optimization: Replace O(N^2) pixel-by-pixel assignments (`cv::Mat::at`)
    // with vectorized `cv::rectangle` calls using `cv::FILLED` for significant
    // performance improvement in hot loops.
...
void PathPlanner::maskOccupancyGrid(cv::Mat &grid, const cv::Rect &roi,
  ...
  // Optimization: Replace element-wise conditional assignment loop with
  // vectorized `cv::Mat::setTo` for better performance.
